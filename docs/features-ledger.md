...
---------|---------|---------|---------|
        MVP      Alpha     Beta  v1.0.0 
weight=9.74 features=10 kloc=24.2
```
...
| ----- | ----------------------------------------------------- | ------------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------- | ------ | ---------------- | ------------ | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | ------------------------------ | ---------------------------------------------- |
|🅱️ | [Core Job Queue](../README.md) | Core/Runtime | [Roadmap](../ROADMAP.md), [FIFO design](../design/per-key-fifo.md) | [queueclient](../pkg/queueclient), [queueworker](../pkg/queueworker), [queuekeys](../pkg/queuekeys), [internal/queue](../internal/queue), [worker](../internal/worker), [producer](../internal/producer) | 5.8 | Beta | 100% (conf: high) | `██████████` | External Go clients enqueue byte-exact jobs and register required handlers through a public worker API; live handler removal pauses and restores in-flight claims safely; compact ownership-only heartbeats, durable ordered-claim recovery metadata, and bounded intake feed crash-safe per-key FIFO with UTF-8-stable ordering, mutation-safe transitions, tail-bound reaper recovery, generation-and-length-bound DLQ selection and pagination, recorded-priority restoration, lease-bound handler cancellation, disjoint worker and ordered keyspaces, static/generated and per-job role validation, deterministic priority aliases, and digest-filtered statistics and cleanup. | Production soak, operational dashboards, and consumer adoption. | Default/race core coverage; external imports; live handler clear/claim restoration/resume; 1 MiB heartbeat-value regression; retry/DLQ; invalid encoding and wrong-type preservation; identical-prepend and same-length ABA selection; stale/malformed pagination cursors; mixed and custom-only priority restoration; static, generated-key, per-job, processing-pattern, heartbeat-pattern, and dynamic-keyspace-overlap matrices; malformed-discard failure recovery plus malformed- and valid-tail interleavings; strict HTTP JSON; broad-pattern purge preservation; digest-only stats; lease-loss cancellation; real-Redis strict order, concurrency, fairness, lease, race, and 10k-key tests. | All 56 exact-head review findings are resolved; stale test/tool documentation is reconciled. Live issue audit: zero open. |
