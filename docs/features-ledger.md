...
---------|---------|---------|---------|
        MVP      Alpha     Beta  v1.0.0 
weight=9.64 features=10 kloc=23.3
```
...
| ----- | ----------------------------------------------------- | ------------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------- | ------ | ---------------- | ------------ | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | ------------------------------ | ---------------------------------------------- |
|🅱️ | [Core Job Queue](../README.md) | Core/Runtime | [Roadmap](../ROADMAP.md), [FIFO design](../design/per-key-fifo.md) | [queueclient](../pkg/queueclient), [queueworker](../pkg/queueworker), [queuekeys](../pkg/queuekeys), [internal/queue](../internal/queue), [worker](../internal/worker), [producer](../internal/producer) | 5.4 | Beta | 100% (conf: high) | `██████████` | External Go clients enqueue byte-exact jobs and register required handlers through a public worker API; live handler removal pauses and restores in-flight claims safely; bounded intake and atomic batches feed crash-safe per-key FIFO with UTF-8-stable ordering, mutation-safe transitions, generation-and-length-bound DLQ selection, lease-bound handler cancellation, static/generated and per-job role validation, processing-scan isolation, deterministic priority aliases, and digest-filtered statistics and cleanup. | Production soak, operational dashboards, and consumer adoption. | Default/race core coverage; external imports; live handler clear/claim restoration/resume; retry/DLQ; invalid encoding and wrong-type preservation; identical-prepend and same-length ABA selection; static, generated-key, per-job, processing-pattern, and heartbeat-pattern role matrices; malformed-tail interleaving; strict HTTP JSON; broad-pattern purge preservation; worker-role separation; digest-only stats; lease-loss cancellation; real-Redis strict order, concurrency, fairness, lease, race, and 10k-key tests. | All 48 exact-head review findings are resolved; stale test/tool documentation is reconciled. Live issue audit: zero open. |
