...
---------|---------|---------|---------|
        MVP      Alpha     Beta  v1.0.0 
weight=9.51 features=10 kloc=21.4
```
...
| ----- | ----------------------------------------------------- | ------------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------- | ------ | ---------------- | ------------ | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | ------------------------------ | ---------------------------------------------- |
|🅱️ | [Core Job Queue](../README.md) | Core/Runtime | [Roadmap](../ROADMAP.md), [FIFO design](../design/per-key-fifo.md) | [queueclient](../pkg/queueclient), [queueworker](../pkg/queueworker), [queuekeys](../pkg/queuekeys), [internal/queue](../internal/queue), [worker](../internal/worker), [producer](../internal/producer) | 4.1 | Beta | 100% (conf: high) | `██████████` | External Go clients enqueue byte-exact jobs and register required handlers through a public worker API; bounded intake and atomic batches feed crash-safe per-key FIFO, including ordered DLQ requeue. | Production soak, operational dashboards, and consumer adoption. | Default/race core coverage; external enqueue and worker imports; retry/DLQ; wrong-type atomicity; key-layout validation; real-Redis strict order, concurrency, fairness, lease, race, and 10k-key tests. | Missing handlers fail before consumption; ordered mutations preflight types; public retry counters and reserved aliases are normalized safely. Live issue audit: zero open. |
...
|✅ | Repository Quality Gates | DevEx/CI | — | [CI workflows](../.github/workflows), [Markdownlint policy](../.markdownlint.yaml) | 0.0 | V1 | 100% (conf: high) | ██████████ | Repository-wide Go vet, Markdownlint, request-ID lint, README document-reference checks, and reachable-vulnerability scans pass. | Preserve the full-corpus gates; clear legacy ShellCheck findings; migrate checkout/setup-go actions for Node 24 runners. | `go vet ./...`; `make lint`; default README reference regression; Markdownlint 0.14.0 checks 136 tracked files; `govulncheck` reports zero reachable vulnerabilities. | MD013, MD028, and MD036 are intentionally disabled for repository style. |
