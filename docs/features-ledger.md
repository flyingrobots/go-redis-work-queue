...
---------|---------|---------|---------|
        MVP      Alpha     Beta  v1.0.0 
weight=9.64 features=10 kloc=23.4
```
...
| ----- | ----------------------------------------------------- | ------------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------- | ------ | ---------------- | ------------ | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | ------------------------------ | ---------------------------------------------- |
|🅱️ | [Core Job Queue](../README.md) | Core/Runtime | [Roadmap](../ROADMAP.md), [FIFO design](../design/per-key-fifo.md) | [queueclient](../pkg/queueclient), [queueworker](../pkg/queueworker), [queuekeys](../pkg/queuekeys), [internal/queue](../internal/queue), [worker](../internal/worker), [producer](../internal/producer) | 5.5 | Beta | 100% (conf: high) | `██████████` | External Go clients enqueue byte-exact jobs and register required handlers through a public worker API; live handler removal pauses and restores in-flight claims safely; compact ownership-only heartbeats and bounded intake feed crash-safe per-key FIFO with UTF-8-stable ordering, mutation-safe transitions, generation-and-length-bound DLQ selection, lease-bound handler cancellation, disjoint worker keyspaces, static/generated and per-job role validation, deterministic priority aliases, and digest-filtered statistics and cleanup. | Production soak, operational dashboards, and consumer adoption. | Default/race core coverage; external imports; live handler clear/claim restoration/resume; 1 MiB heartbeat-value regression; retry/DLQ; invalid encoding and wrong-type preservation; identical-prepend and same-length ABA selection; static, generated-key, per-job, processing-pattern, heartbeat-pattern, and dynamic-keyspace-overlap matrices; malformed-tail interleaving; strict HTTP JSON; broad-pattern purge preservation; digest-only stats; lease-loss cancellation; real-Redis strict order, concurrency, fairness, lease, race, and 10k-key tests. | All 50 exact-head review findings are resolved; stale test/tool documentation is reconciled. Live issue audit: zero open. |
|🅱️ | Admin API v1 (HTTP) | Platform/API | — | [internal/admin-api](../internal/admin-api) | 6.2 | Beta | 90% (conf: high) | `█████████░` | Endpoints, OpenAPI, authentication, and endpoint-catalog authorization are wired; enqueue requires `queue:write` and one complete JSON document; DLQ mutations use constant-time generation-and-length versions in per-entry handles; destructive routes match exactly. | TUI switchover for Stats; expand e2e; gRPC decision; soak/chaos; share port-forward helper across deployment scripts; add policy-as-code checks for manifest security/secret rules. | Unit + integration; viewer/operator authorization; trailing-JSON rejection; deceptive-route denial; duplicate-ID, identical-envelope, stale-version, and same-length ABA DLQ selection. | Productionize defaults; audit destructive ops. |
...
|🅱️ | Reaper | Maintenance | — | [internal/reaper](../internal/reaper) | 0.4 | Beta | 90% (conf: high) | █████████░ | TTL cleanup uses content-bound malformed-tail removal and excludes managed static keys from processing scans. | Tune policies; monitoring; long-run soak. | Unit, deterministic race interleaving, and configuration matrices pass. | Keep safe defaults. |
...
---------|---------|---------|---------|
        MVP      Alpha     Beta  v1.0.0 
weight=10.53 features=9 kloc=29.8
```
...
|🅰️ | Bench (Basic) | Testing | — | [internal/admin](../internal/admin), [internal/tui](../internal/tui) | 4.6 | Alpha | 60% (conf: med) | ██████░░░░ | Running; progress UI present; baseline delta pending. | Baseline from initial completed list; cancel; ETA/throughput; guardrails. | Manual + some unit. | Guardrails for high rates. |
