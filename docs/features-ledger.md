...
|✅ | Repository Quality Gates | DevEx/CI | — | [CI workflows](../.github/workflows), [Markdownlint policy](../.markdownlint.yaml) | 0.0 | V1 | 100% (conf: high) | ██████████ | Repository-wide Go vet, Markdownlint, and reachable-vulnerability scans pass. | Preserve the full-corpus gates and evaluate link checking separately. | `go vet ./...`; Markdownlint checks 136 tracked files with zero findings; `govulncheck` reports zero reachable vulnerabilities. | MD013, MD028, and MD036 are intentionally disabled for repository style. |
