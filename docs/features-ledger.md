...
---------|---------|---------|---------|
        MVP      Alpha     Beta  v1.0.0 
weight=9.79 features=10 kloc=25.2
```
...
| ----- | ----------------------------------------------------- | ------------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------- | ------ | ---------------- | ------------ | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | ------------------------------ | ---------------------------------------------- |
|🅱️ | [Core Job Queue](../README.md) | Core/Runtime | [Roadmap](../ROADMAP.md), [FIFO design](../design/per-key-fifo.md) | [queueclient](../pkg/queueclient), [queueworker](../pkg/queueworker), [queuekeys](../pkg/queuekeys), [internal/queue](../internal/queue), [worker](../internal/worker), [producer](../internal/producer) | 6.6 | Beta | 100% (conf: high) | `██████████` | External Go clients enqueue byte-exact jobs and register required handlers through a public worker API; live handler removal pauses and restores in-flight claims safely or stops on ambiguous restoration; compact ownership-only heartbeats, isolated handler payloads, durable ordered-claim recovery metadata, occupied-worker claim deferral, fail-safe unordered destination writes, pool-wide typed safe-stop propagation, and bounded intake feed crash-safe per-key FIFO with UTF-8-stable ordering, mutation-safe transitions, tail-bound reaper recovery, generation-and-length-bound DLQ selection and pagination, recorded-priority restoration, heartbeat-uncertainty cancellation, disjoint worker and ordered keyspaces, static/generated and per-job role validation, canonical queue mappings, canonical worker discovery and digest-filtered cleanup/recovery/statistics, ready-ring digest validation, deterministic priority aliases, and required completed and dead-letter destinations. | Production soak, operational dashboards, and consumer adoption. | Default/race core coverage; external imports and public safe-stop error; live handler clear/claim restoration/resume and restore-failure injection; sibling cancellation; 1 MiB heartbeat-value and retry-payload mutation regressions; occupied-processing claim deferral; rejected completion/retry/DLQ write preservation, no-second-dequeue regression, and completion-cleanup fault injection; retry/DLQ; invalid encoding and wrong-type preservation; identical-prepend and same-length ABA selection; stale/malformed pagination cursors; mixed and custom-only priority restoration; static, generated-key, per-job, processing-pattern, heartbeat-pattern, and dynamic-keyspace-overlap matrices; malformed-discard failure recovery plus malformed- and valid-tail interleavings; strict HTTP JSON; broad-pattern purge/reaper/stats/worker-listing preservation; malformed ready-token rejection; five-case canonical queue-mapping and empty terminal-destination validation; digest-only stats; lease-loss cancellation; real-Redis strict order, concurrency, fairness, lease, race, and 10k-key tests. | Twenty-one exact-head review passes produced 70 findings; all 70 are resolved. Stale test/tool documentation is reconciled. Live issue audit: zero open. |
|🅱️ | Admin API v1 (HTTP) | Platform/API | — | [internal/admin-api](../internal/admin-api) | 6.4 | Beta | 90% (conf: high) | `█████████░` | Endpoints, OpenAPI, authentication, and endpoint-catalog authorization are wired; enqueue requires `queue:write`, one complete JSON document, and byte-valid Unicode ordering identity; DLQ selection handles and page cursors use constant-time generation-and-length versions, omitted destinations restore recorded priorities, invalid/stale cursors are classified, destructive routes match exactly, and worker discovery admits only canonical generated worker identities. | TUI switchover for Stats; expand e2e; gRPC decision; soak/chaos; share port-forward helper across deployment scripts; add policy-as-code checks for manifest security/secret rules. | Unit + integration; viewer/operator authorization; trailing-JSON and malformed UTF-8/surrogate rejection; deceptive-route denial; duplicate-ID, identical-envelope, stale-version, same-length ABA, priority restoration, version-bound pagination, and broad-pattern worker-discovery coverage. | Productionize defaults; audit destructive ops. |
...
|🅱️ | Reaper | Maintenance | — | [internal/reaper](../internal/reaper) | 0.7 | Beta | 90% (conf: high) | █████████░ | TTL cleanup binds ordinary malformed removal and requeue to the exact inspected tail, uses durable claim metadata to settle malformed ordered work, routes valid ordered recovery separately, excludes managed static keys from processing scans, and admits only canonical generated worker IDs from broad scan patterns. | Tune policies; monitoring; long-run soak. | Unit, broad-pattern shared-key preservation, deterministic failed-discard and stale-tail interleavings, repeated race, and configuration matrices pass. | Keep safe defaults. |
...
|🅱️ | Config | Core/Runtime | — | [internal/config](../internal/config) | 0.8 | Beta | 90% (conf: high) | █████████░ | Strict YAML loading, validated defaults, unsupported-key rejection, canonical queue aliases and Redis keys, required completed and dead-letter destinations, and disjoint worker/ordered generated keyspaces. | Expand field validation; document env overrides and compatibility policy. | Defaults, validation, keyspace-intersection, example, dead-key, five-case whitespace/blank queue-mapping matrix, and empty completed/DLQ regression tests. | Unsupported features cannot appear silently enabled. |
...
---------|---------|---------|---------|
        MVP      Alpha     Beta  v1.0.0 
weight=10.54 features=9 kloc=30.3
```
...
|🅰️ | Bench (Basic) | Testing | — | [internal/admin](../internal/admin), [internal/tui](../internal/tui) | 5.0 | Alpha | 60% (conf: med) | ██████░░░░ | Running; progress UI present; baseline delta pending. | Baseline from initial completed list; cancel; ETA/throughput; guardrails. | Manual + some unit. | Guardrails for high rates. |
