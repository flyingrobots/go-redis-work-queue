...
---------|---------|---------|---------|
        MVP      Alpha     Beta  v1.0.0 
weight=9.75 features=10 kloc=24.4
```
...
| ----- | ----------------------------------------------------- | ------------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------- | ------ | ---------------- | ------------ | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | ------------------------------ | ---------------------------------------------- |
|🅱️ | [Core Job Queue](../README.md) | Core/Runtime | [Roadmap](../ROADMAP.md), [FIFO design](../design/per-key-fifo.md) | [queueclient](../pkg/queueclient), [queueworker](../pkg/queueworker), [queuekeys](../pkg/queuekeys), [internal/queue](../internal/queue), [worker](../internal/worker), [producer](../internal/producer) | 6.0 | Beta | 100% (conf: high) | `██████████` | External Go clients enqueue byte-exact jobs and register required handlers through a public worker API; live handler removal pauses and restores in-flight claims safely; compact ownership-only heartbeats, isolated handler payloads, durable ordered-claim recovery metadata, occupied-worker claim deferral, and bounded intake feed crash-safe per-key FIFO with UTF-8-stable ordering, mutation-safe transitions, tail-bound reaper recovery, generation-and-length-bound DLQ selection and pagination, recorded-priority restoration, heartbeat-uncertainty cancellation, disjoint worker and ordered keyspaces, static/generated and per-job role validation, canonical worker and digest-filtered cleanup, deterministic priority aliases, and digest-filtered statistics. | Production soak, operational dashboards, and consumer adoption. | Default/race core coverage; external imports; live handler clear/claim restoration/resume; 1 MiB heartbeat-value and retry-payload mutation regressions; occupied-processing claim deferral; retry/DLQ; invalid encoding and wrong-type preservation; identical-prepend and same-length ABA selection; stale/malformed pagination cursors; mixed and custom-only priority restoration; static, generated-key, per-job, processing-pattern, heartbeat-pattern, and dynamic-keyspace-overlap matrices; malformed-discard failure recovery plus malformed- and valid-tail interleavings; strict HTTP JSON; broad-pattern purge preservation; digest-only stats; lease-loss cancellation; real-Redis strict order, concurrency, fairness, lease, race, and 10k-key tests. | All 59 exact-head review findings are resolved; stale test/tool documentation is reconciled. Live issue audit: zero open. |
...
|🅱️ | Reaper | Maintenance | — | [internal/reaper](../internal/reaper) | 0.6 | Beta | 90% (conf: high) | █████████░ | TTL cleanup binds ordinary malformed removal and requeue to the exact inspected tail, uses durable claim metadata to settle malformed ordered work, routes valid ordered recovery separately, and excludes managed static keys from processing scans. | Tune policies; monitoring; long-run soak. | Unit, deterministic failed-discard and stale-tail interleavings, repeated race, and configuration matrices pass. | Keep safe defaults. |
