...
> | Confidence | Remarks |
> |------------|---------|
> | 10 | Integration & security examples now rely on explicit package paths. |
>
> ## Lesson Learned
>
> Using package paths keeps commands portable and scriptable without `cd` gymnastics.
>
> ## What did you do to address this feedback?
>
> Replaced the `cd ... && go test ./*.go` pattern with direct `go test -v ./test/integration` and updated the security example to a package-aware pattern run.
>
> ## Regression Avoidance Strategy
>
> Future additions can reuse the same structure for new test suites.
>
> ## Notes
>
> None.
...
> | Confidence | Remarks |
> |------------|---------|
> | 10 | Benchmark docs now isolate benches with `-run '^$'` and module-wide paths. |
>
> ## Lesson Learned
>
> Always disable tests when running benches so results stay clean.
>
> ## What did you do to address this feedback?
>
> Updated the commands to `go test -run '^$' -bench=. ./...` and added a pattern-specific variant for targeted benchmarks.
>
> ## Regression Avoidance Strategy
>
> Documenting the `-run '^$'` idiom ensures we don't accidentally execute unit tests during benchmarking.
>
> ## Notes
>
> None.
...
> | Confidence | Remarks |
> |------------|---------|
> | 9 | Coordinator task now derives dependencies from the generated worker tasks instead of a handwritten list. |
>
> ## Lesson Learned
>
> Scripts that emit related tasks should derive their wiring from the same source of truth to stay in sync.
>
> ## What did you do to address this feedback?
>
> Replaced the static dependency array with a list comprehension that collects every generated worker task ID.
>
> ## Regression Avoidance Strategy
>
> Any future change to the number of workers automatically propagates to the coordinator task without extra edits.
>
> ## Notes
>
> None.
