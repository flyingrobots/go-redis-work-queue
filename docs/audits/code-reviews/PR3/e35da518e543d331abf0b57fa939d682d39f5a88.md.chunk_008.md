...
> | Confidence | Remarks |
> |------------|---------|
> | 9 | Added a public re-export at `pkg/anomaly-radar-slo-budget` and updated docs to reference the stable path. |
>
> ## Lesson Learned
>
> Provide an explicit public entrypoint once the API settles so downstream consumers aren’t forced to rely on `internal/` packages.
>
> ## What did you do to address this feedback?
>
> Introduced `pkg/anomaly-radar-slo-budget/export.go` that re-exports the internal types/functions and switched all examples to import from the new package.
>
> ## Regression Avoidance Strategy
>
> Future surface changes require updating a single wrapper; docs and clients stay aligned with the supported import path.
>
> ## Notes
>
> None.
...
> | Confidence | Remarks |
> |------------|---------|
> | 8 | Added a global auth/error section, per-endpoint scope tables, and explicit Content-Type/error listings. |
>
> ## Lesson Learned
>
> Documenting scopes and the shared error envelope once keeps each endpoint description concise yet complete.
>
> ## What did you do to address this feedback?
>
> Created a consolidated “Authentication & Error Handling” section, annotated every endpoint with scope, status codes, and Content-Type, and noted the standard error envelope and timestamp format beneath each response.
>
> ## Regression Avoidance Strategy
>
> New endpoints must link into the shared tables; failing tests will surface missing scope metadata in docs review.
>
> ## Notes
>
> None.
...
> | Confidence | Remarks |
> |------------|---------|
> | 8 | Added explicit RFC3339/UTC notes under every response example. |
>
> ## Lesson Learned
>
> Timestamp conventions are easy to miss unless they’re reiterated alongside the samples.
>
> ## What did you do to address this feedback?
>
> Appended “All timestamps are RFC3339 in UTC (trailing `Z`).” beneath each response block so readers see the rule without scrolling.
>
> ## Regression Avoidance Strategy
>
> Future response additions must include the same note; reviewers now have an obvious checklist.
>
> ## Notes
>
> None.
...
> | Confidence | Remarks |
> |------------|---------|
> | 8 | Documented PUT as a full replace with explicit validation, 400/422 behaviours, and immutability notes. |
>
> ## Lesson Learned
>
> Consumers need to know whether configuration updates overwrite or merge fields and what validation to expect.
>
> ## What did you do to address this feedback?
>
> Clarified that PUT replaces the configuration, listed validation constraints, noted that unknown fields trigger 400 and business-rule failures yield 422 with details, and flagged fields that require a restart.
>
> ## Regression Avoidance Strategy
>
> Any future config changes must update the validation bullets so docs and behaviour stay aligned.
>
> ## Notes
>
> None.
...
> | Confidence | Remarks |
> |------------|---------|
> | 8 | Documented pagination defaults, limits, and `next_cursor` contract alongside the updated handlers. |
>
> ## Lesson Learned
>
> API docs must spell out cursor semantics the moment the backend supports them.
>
> ## What did you do to address this feedback?
>
> Added default/max values for `max_samples`, described the opaque `next_cursor`, updated response samples, and linked the client snippet that loops until `next_cursor` is empty.
>
> ## Regression Avoidance Strategy
>
> Future changes to pagination must update both the docs and OpenAPI spec, which now share the same fields.
>
> ## Notes
>
> None.
