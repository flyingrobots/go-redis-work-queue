...
test_documentation() {
    ...
    local doc_path="docs/design/f007-design.md"

    if [[ -f "$doc_path" ]]; then
        success "Exactly-once patterns design document found"

        # Check for key sections
        if grep -q -i "tradeoffs\|failure\|modes" "$doc_path"; then
        ...
        error "Documentation not found: $doc_path"
        return 1
    fi
