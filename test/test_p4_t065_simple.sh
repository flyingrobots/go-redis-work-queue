...
# Simplified acceptance test for P4.T065
...
if [ -f "docs/design/f026-design.md" ]; then
    check_test 0 "Architecture document exists"

    LINES=$(wc -l < "docs/design/f026-design.md")
...
if [ -f "docs/api/f026-openapi.yaml" ]; then
    check_test 0 "OpenAPI specification exists"

    LINES=$(wc -l < "docs/api/f026-openapi.yaml")
...
if [ -f "docs/schemas/f026-schema.json" ]; then
    check_test 0 "JSON Schema file exists"

    LINES=$(wc -l < "docs/schemas/f026-schema.json")
    if [ "$LINES" -gt 1000 ]; then
