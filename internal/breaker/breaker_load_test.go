...
func TestBreakerHalfOpenSingleProbeUnderLoad(t *testing.T) {
    ...
    cb.Record(false)
    cb.Record(false)
    if cb.State() != Open { t.Fatal("expected open after 2 failures") }
    ...
    cb.Record(false)
    if cb.State() != Open { t.Fatalf("expected open after failed probe, got %v", cb.State()) }
    ...
    cb.Record(true)
    if cb.State() != Closed { t.Fatalf("expected closed after successful probe, got %v", cb.State()) }
}
