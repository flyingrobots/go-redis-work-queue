...
func initialModel(cfg *config.Config, rdb *redis.Client, logger *zap.Logger, refreshEvery time.Duration) model {
    ...
    bi := textinput.New(); bi.Placeholder = "count"; bi.SetValue("1000")
    br := textinput.New(); br.Placeholder = "rate"; br.SetValue("500")
    bp := textinput.New(); bp.Placeholder = "priority"; bp.SetValue(cfg.Producer.DefaultPriority)
    bt := textinput.New(); bt.Placeholder = "timeout (s)"; bt.SetValue("60")

    fi := textinput.New(); fi.Placeholder = "filter"; fi.CharLimit = 64
