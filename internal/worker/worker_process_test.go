...
func TestProcessJobSuccess(t *testing.T) {
    ...
    if !ok { t.Fatalf("expected success") }
    if n, _ := rdb.LLen(ctx, cfg.Worker.CompletedList).Result(); n != 1 { t.Fatalf("expected completed 1, got %d", n) }
}
...
func TestProcessJobRetryThenDLQ(t *testing.T) {
    ...
    ctx := context.Background()
    ok := w.processJob(ctx, workerID, cfg.Worker.Queues["low"], procList, hbKey, payload)
    if ok { t.Fatalf("expected failure") }
    // After one retry allowed, it should have been requeued to low
    if n, _ := rdb.LLen(ctx, cfg.Worker.Queues["low"]).Result(); n != 1 { t.Fatalf("expected requeued 1, got %d", n) }
